...
public class DriveController {
    ...
    private void onGameOver(GameOverReason reason) {
        ...
        baseLoad.registerPlay();
        switch (reason) {
            case ActionComplete:
                if (baseLoad instanceof EndLoad) {//在终点地垫上
                    //在终点地垫上
            ...
            if (action != null) {
                lock.unLock(key, new LockListener<Additional>() {
                    ...
                    public void onSuccess(Additional value, Object param) {
                        Director.getInstance().director(BaseLoad.ON_UNLOCK_SUCCESS, () -> {
                            Director.getInstance().reset();
                            setComputeExitPathState();
                            reset();
                        });
                    ...
                    public void onFail(final Additional value, final Object param) {
                        Director.getInstance().director(BaseLoad.ON_UNLOCK_FAIL, () -> {
                            pause();
                            if (mListener != null) {
                                if (value == null) {
